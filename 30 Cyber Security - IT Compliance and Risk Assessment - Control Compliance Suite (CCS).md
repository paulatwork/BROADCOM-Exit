...
- Rewrote the section as a narrative, replacing the numbered list and bold sub-headings with continuous prose.
- Added a caution that IBM has ended service for QRadar Vulnerability Manager, so the IBM route relies on third-party scanning connectors.
- Recorded that no named organisation has publicly left the product.
- Moved all citations, previously a numbered list, to the Sources: Analyst reviews and exit strategy section.
